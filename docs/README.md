[Previous](../README.md) | [Index](INDEX.md)

# Documentation Index

This page is the table of contents for all framework documentation.

## Fundamentals

- [Web and HTMX Primer](fundamentals/01-web-and-htmx-primer.md)
- [SimplyPages Mental Model](fundamentals/02-simplypages-mental-model.md)
- [CSS Fundamentals](fundamentals/03-css-fundamentals.md)

## Getting Started

- [Getting Started Overview](getting-started/README.md)
- [Installation and First Static Page](getting-started/01-installation-and-first-static-page.md)
- [Dynamic Pages with SlotKey and RenderContext](getting-started/02-dynamic-pages-with-slotkey-rendercontext.md)
- [Editing System First Implementation](getting-started/03-editing-system-first-implementation.md)

## Core

- [Components, HtmlTag, and Module Lifecycle](core/01-components-htmltag-and-module-lifecycle.md)
- [Layout: Page, Row, Column, Grid](core/02-layout-page-row-column-grid.md)
- [Template, RenderContext, and SlotKey Reference](core/03-template-rendercontext-slotkey-reference.md)
- [Rendering Pipeline: High and Low Level](core/04-rendering-pipeline-high-and-low-level.md)
- [CSS: Defaults, Overrides, and Structure](core/05-css-defaults-overrides-and-structure.md)
- [Shell Project Structure and Asset Load Chain](core/06-shell-project-structure-and-asset-load-chain.md)
- [Mobile Rendering Model and Responsive Behavior](core/07-mobile-rendering-model-and-responsive-behavior.md)

## Patterns

- [Static Page Serving Patterns](patterns/01-static-page-serving-patterns.md)
- [Dynamic Fragment Caching Patterns](patterns/02-dynamic-fragment-caching-patterns.md)
- [HTMX Endpoint and Swap Patterns](patterns/03-htmx-endpoint-and-swap-patterns.md)
- [Editing Workflows: Owner, User, Approval](patterns/04-editing-workflows-owner-user-approval.md)

## Security

- [Security Boundaries and Safe Rendering](security/01-security-boundaries-and-safe-rendering.md)
- [AuthWrapper and AuthorizationChecker Integration](security/02-authwrapper-authorizationchecker-integration.md)

## Operations

- [Performance, Threading, and Cache Lifecycles](operations/01-performance-threading-and-cache-lifecycles.md)
- [Testing and Troubleshooting Playbook](operations/02-testing-and-troubleshooting-playbook.md)

## Reference

- [Components and Modules Catalog](reference/components-and-modules-catalog.md)
- [Builders Reference: Shell, Navigation, Banner, Account Bar](reference/builders-shell-nav-banner-accountbar.md)
- [Editing API Reference](reference/editing-api-reference.md)
